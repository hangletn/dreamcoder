...
class TemporalFeatureExtractor(nn.Module):
    ...
    def __init__(self, tasks, testingTasks=None, cuda=False, H=64, **kwargs):
        ...
        self.helmholtzEvalTimeout =  0.01 #helmholtzEvaluationTimeout #0.01
    ...
    def _encode_single_example(self, example, meta):
        ...
        # Ensure meta is always a dict (use defaults if None or missing keys)
        ...
        z_with_obstacle_and_meta = torch.cat([z, obstacle_pos, meta_tensor], dim=0) # (H+5,)
        z_final = self.obstacle_mlp(z_with_obstacle_and_meta) # (H,)
        return z_final
    ...
    def featuresOfTask(self, task):
        # Get meta info from task if available
        meta = getattr(task, 'meta', None)
        # Ensure meta is a dict (not None or empty)
        if meta is None or not isinstance(meta, dict):
            meta = {"ball_radius": 25.0, "box_size": 50.0, "ramp_theta": -0.2914567944778671}
