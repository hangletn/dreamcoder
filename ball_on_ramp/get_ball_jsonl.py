...
def make_windows(xs, ys, h, stride=2):
    ...
    - Returns windows without next step (for window_internal_bools)
    """
    out = []
    n = len(xs)
        ...
        for t in t_vals:
            ...
            for e in range(args.num_examples):
                ...
                if capture_video:
                    ...
                    frames = None
                for start_i, xw, yw in make_windows(xs, ys, args.window_h):
                    # Round positions if requested
                    if args.round_decimals is not None:
                        xw = [round(x, args.round_decimals) for x in xw]
                        yw = [round(y, args.round_decimals) for y in yw]
                    
                    # Generate labels based on movement within window
                    labels = window_internal_bools(xw, yw, eps=args.eps, use_exact_equality=args.use_exact_equality)
                    ...
                    # Include meta information (ball_radius, box_size, ramp_theta)
                    record = {
                        "pos_x": xw,
                        "pos_y": yw,
                        "obstacle_x": obstacle_x,  # Single value, not list
                        "obstacle_y": obstacle_y,  # Single value, not list
                        "out": {"move_x": labels["move_x"], "move_y": labels["move_y"]},
                        "meta": {
                            "t": t,
                            "dt": args.dt,
                            "start_index": start_i,
